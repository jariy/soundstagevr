...
using System.Linq;
using Valve.VR;
//using Valve.VR.SteamInteractionSystem;
...
public class manipulator : MonoBehaviour
{
    ...
    void Grab(bool on)
    {
        Debug.Log("Grab Action Activated: grap()");
        grabbing = on;
        showTip();
    ...
    public void hapticPulse(ushort hapticPower = 750)
    {
        ...
        {
            SteamVR_Actions.default_Haptic.Execute(0, hapticPower, 200, 1, SteamVR_Input_Sources.LeftHand); ///.Input(controllerIndex).TriggerHapticPulse(hapticPower);
            SteamVR_Actions.default_Haptic.Execute(0, hapticPower, 200, 1, SteamVR_Input_Sources.RightHand);
        }
    ...
    public void bigHaptic(ushort hapticPower = 750, float dur = .1f)
    {
        ...
        SteamVR_Actions.default_Haptic.Execute(0, dur, hapticPower, 1, SteamVR_Input_Sources.LeftHand); ///.Input(controllerIndex).TriggerHapticPulse(hapticPower);
        SteamVR_Actions.default_Haptic.Execute(0, dur, hapticPower, 1, SteamVR_Input_Sources.RightHand);
        Debug.Log("HapticFeed Activated");
    }
    ...
    void Update()
    {
        ...
        */
        secondaryOculusButtonUpdate(); //using Oculus Controllers
        if ( SteamVR_Actions.default_GrabPinch.stateDown /*|| SteamVR_Actions.default_GrabGrip.state*/) //triggerButtonDown)
        {
        ...
        if (SteamVR_Actions.default_GrabPinch.stateUp /*|| !(SteamVR_Actions.default_GrabGrip.state)*/) //triggerButtonUp)
        {
            activeTip.SetActive(false);
        ...
        if (SteamVR_Actions.default_GrabGrip.stateDown) _menuspawn.togglePad(); //fix this !!!!
